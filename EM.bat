...
:: En este punto, el Equipo Apolo empieza a realizar los cambios solicitados por el equipo Tilinesfc
::
rem Calculo de distacnia y tiempo aproximado

set /a distaciaTotal=%destino%-%origen%
if %distanciaTotal& lss 0 set /a distanciaTotal*=-1

set /a tiempoTotal=(%distanciaTotal% * 2) + 2
echo La distancia estimada del viaje de la estacion %origen% a la estacion %destino% es %distanciaTotal% kilometros.
echo
echo El tiempo estimado de viaje a la estacion %origen% a la estacion %destino% es %tiempoTotal% minutos.
